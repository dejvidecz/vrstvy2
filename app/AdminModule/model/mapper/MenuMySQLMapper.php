...
class MenuMySQLMapper implements IMenuMapper {
    ...
    public function delete(MenuEntity $menu) {
        $this->database->table($this->tableName)->where('id = ?', $menu->getId());
    }

    public function deleteMenuItem(MenuItemEntity $menuItem) {
        $this->database->table($this->tableNameItem)->where('id = ?', $menuItem->getId());
    }

    public function deleteMenuType(MenuTypeEntity $menuType) {
        $this->database->table($this->tableNameType)->where('id = ?', $menuType->getId());
    }
